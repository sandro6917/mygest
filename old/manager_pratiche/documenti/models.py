...
from core.storages import NASPathStorage
...
def documento_upload_to(instance, filename):
    ...
    elif getattr(instance, "fascicolo_id", None):
        cli = instance.fascicolo.cliente if instance.fascicolo_id else None

    def _cli_code(cli_obj):
        if not cli_obj:
            return "VARIE"
        cf = (cli_obj.codice_fiscale or "").upper()
        if cli_obj.tipo == "PG":
            base = cf[-5:] if cf.isdigit() and len(cf) >= 5 else (cli_obj.ragione_sociale or "CLI").split()[0][:8].upper()
        else:
            base = f"{(cli_obj.cognome or 'CLI')[:3]}{(cli_obj.nome or '')[:3]}".upper()
        return base.replace(" ", "")

    cli_code = _cli_code(cli)
...
class Documento(models.Model):
    ...
    cliente = models.ForeignKey(Anagrafica, on_delete=models.PROTECT, related_name="documenti")
    ...
    def _generate_codice(self, seq: int) -> str:
        ...
        tipo_code = self.tipo.codice.upper()
        data_str = self.data_documento.strftime("%Y%m%d")
        pattern = self.tipo.pattern_codice
        try:
            return pattern.format(CLI=cli_code, TIPO=tipo_code, DATA=data_str, SEQ=seq)
        except Exception:
            return f"{cli_code}_{tipo_code}_{data_str}_{seq:03d}"

    def _build_path(self) -> str:
        """Percorso ASSOLUTO del contenitore del documento nel NAS (unificato con Fascicolo)."""
        # Se legato a un fascicolo, usa il path del fascicolo
        if self.fascicolo_id and self.fascicolo.path_archivio:
            return self.fascicolo.path_archivio
        # Altrimenti costruisci come per i fascicoli
        cli_code = self._cliente_code(self.cliente)
        tit_parts = []
        node = self.titolario_voce
        while node:
            tit_parts.insert(0, f"{node.codice} - {node.titolo}")
            node = node.parent
        anno = self.data_documento.year
        return ensure_archivio_path(cli_code, tit_parts, anno)
    ...
    def protocolla_entrata(self, *, quando=None, operatore=None, da_chi: str = "",
                           ubicazione: Optional[Ubicazione] = None, causale: str = "", note: str = ""):
        return MovimentoProtocollo.registra_entrata(
            self, quando=quando, operatore=operatore,
            da_chi=da_chi, ubicazione=ubicazione,
...
# -----------------------------
#  Protocollo / Movimenti
# -----------------------------
class ProtocolloCounter(models.Model):
    """
    Mantiene l'ultimo numero assegnato per (cliente, anno, direzione).
    Garantisce incrementi atomici con select_for_update.
    """
    DIREZIONE = (("IN", "Entrata"), ("OUT", "Uscita"))
    cliente = models.ForeignKey(Anagrafica, on_delete=models.CASCADE, related_name="protocol_counters")
...
class DocumentoCounter(models.Model):
    cliente = models.ForeignKey(Anagrafica, on_delete=models.CASCADE, related_name="documento_counters")
    anno = models.PositiveIntegerField()
    last_number = models.PositiveIntegerField(default=0)
...
class MovimentoProtocollo(models.Model):
    ...
    documento = models.ForeignKey(Documento, on_delete=models.CASCADE, related_name="movimenti")
    cliente = models.ForeignKey(Anagrafica, on_delete=models.PROTECT, related_name="movimenti")
    direzione = models.CharField(max_length=3, choices=DIREZIONE)
    ...
    ubicazione = models.ForeignKey(Ubicazione, on_delete=models.SET_NULL, null=True, blank=True, help_text=_("Collocazione fisica in entrata"))

    # prestiti/uscite e rientri
    chiuso = models.BooleanField(default=False, help_text=_("Per OUT: flaggato True quando rientra"))
    rientro_di = models.OneToOneField(
        "self", on_delete=models.SET_NULL, null=True, blank=True, related_name="rientrato_con",
        help_text=_("Se questo è un IN di rientro, indica l'OUT chiuso")
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            # indice parziale per OUT aperti (PostgreSQL)
            models.Index(
                fields=["documento"],
                name="mov_out_aperti_idx",
                condition=models.Q(direzione="OUT", chiuso=False),
            ),
        ]
        constraints = [
            ...
            # rientro_di valorizzato solo per IN
            models.CheckConstraint(
                check=models.Q(direzione="IN") | models.Q(rientro_di__isnull=True),
                name="rientro_di_solo_per_IN",
            ),
            # anno coerente con la data
            models.CheckConstraint(
                check=models.Q(anno=models.functions.ExtractYear("data")),
                name="anno_coerente_con_data",
            ),
            # numero positivo
            models.CheckConstraint(
                check=models.Q(numero__gt=0),
                name="numero_maggiore_zero",
            ),
        ]
    ...
    def __str__(self) -> str:
        return f"{self.documento} - {self.protocollo_label} ({self.get_direzione_display()})"

    @property
    def protocollo_label(self) -> str:
        suf = "E" if self.direzione == "IN" else "U"
        return f"{self.anno}/{self.numero:06d}-{suf}"

    # --------- API di dominio ---------
    @classmethod
    @transaction.atomic
    def registra_entrata(cls, documento: Documento, *, quando=None, operatore=None,
                         da_chi: str = "", ubicazione: Optional[Ubicazione] = None,
                         causale: str = "", note: str = "") -> "MovimentoProtocollo":
        quando = quando or timezone.now()
        anno = quando.year

        # chiudi eventuale OUT aperto
        out_aperto = (
            cls.objects.select_for_update()
            .filter(documento=documento, direzione="OUT", chiuso=False)
            .order_by("-data").first()
        )

        numero = cls._next_progressivo(documento.cliente, anno, "IN")
        mov_in = cls.objects.create(
            documento=documento, cliente=documento.cliente,
    ...
    @classmethod
    @transaction.atomic
    def registra_uscita(cls, documento: Documento, *, quando=None, operatore=None,
        ...
        # blocca doppio OUT aperto
        esiste_out_aperto = cls.objects.select_for_update().filter(
            documento=documento, direzione="OUT", chiuso=False
        ).exists()
        if esiste_out_aperto:
            raise ValidationError(_("Esiste già una uscita aperta per questo documento"))

        numero = cls._next_progressivo(documento.cliente, anno, "OUT")
        mov_out = cls.objects.create(
            documento=documento, cliente=documento.cliente,
    ...
    @staticmethod
    @transaction.atomic
    def _next_progressivo(cliente: Anagrafica, anno: int, direzione: str) -> int:
        """Progressivo protocollo per cliente/anno/direzione, safe cross‑DB"""
        for _ in range(5):
            try:
                counter, _ = (
                    ProtocolloCounter.objects.get_or_create(
    ...
    def registra_uscita(cls, documento: Documento, *, quando=None, operatore=None,
        ...
        # lock applicativo: impedisce doppio OUT aperto cross‑DB
        updated = Documento.objects.filter(pk=documento.pk, out_aperto=False).update(out_aperto=True)
        if updated == 0:
            raise ValidationError(_("Esiste già una uscita aperta per questo documento"))
        ...
        try:
            ...
            Documento.objects.filter(pk=documento.pk, out_aperto=True).update(out_aperto=False)
            raise
    ...
    def registra_entrata(cls, documento: Documento, *, quando=None, operatore=None,
        ...
        # OUT aperto (se esiste) viene chiuso; poi libera il lock applicativo
        out_aperto = (
            cls.objects.filter(documento=documento, direzione="OUT", chiuso=False)
            .order_by("-data").first()
        )
        numero = cls._next_progressivo(documento.cliente, anno, "IN")
        mov_in = cls.objects.create(
