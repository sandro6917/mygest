...
from .models import DocumentiTipo, Documento, MovimentoProtocollo, Ubicazione, AttributoDefinizione, AttributoValore
...
class EntrataForm(forms.Form):
    ...
    ubicazione = forms.ModelChoiceField(queryset=Ubicazione.objects.all(), required=False)
    causale = forms.CharField(required=False)
    note = forms.CharField(required=False, widget=forms.Textarea)
...
class DocumentoAdmin(admin.ModelAdmin):
    ...
    def get_form(self, request, obj=None, **kwargs):
        ...
        fascicolo_id = obj.fascicolo_id if obj and obj.fascicolo_id else (request.POST.get("fascicolo") or request.GET.get("fascicolo"))
        if fascicolo_id:
            try:
                f = Fascicolo.objects.only("cliente_id").get(pk=fascicolo_id)
    ...
    def stampa_etichetta_dymo(self, request, queryset):
        ...
        for doc in queryset.select_related("cliente"):
