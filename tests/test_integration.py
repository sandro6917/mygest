...
class TestFlussoDocumentoProtocolloArchivio:
    ...
    def test_flusso_documento_in_uscita(
        ...
        movimento = MovimentoProtocollo.objects.create(
            ...
            anno=timezone.now().year,
            numero=2,
            destinatario_anagrafica=cliente_pf.anagrafica,
        )

        assert movimento.direzione == 'OUT'
        assert movimento.destinatario_anagrafica == cliente_pf.anagrafica
...
class TestFlussoPraticaFascicoloScadenze:
    ...
    def test_creazione_pratica_con_fascicolo_e_documenti(
        self, db, cliente_pf, pratica_tipo, fascicolo_tipo, documento_tipo
    ):
        """
        Test flusso completo:
        1. Crea pratica
        ...
        scadenza = Scadenza.objects.create(
            ...
            data_scadenza=(timezone.now() + timedelta(days=30)).date(),
            stato='attiva',
        )
...
class TestFlussoComunicazioniDocumenti:
    ...
    def test_invio_comunicazione_con_allegato(
        ...
        assert comunicazione.destinatari
    ...
    def test_comunicazione_genera_documento(
        ...
        assert movimento.pk is not None
        assert movimento.documento == documento
...
class TestFlussoPraticaNotaRelazioni:
    ...
    def test_pratica_con_note_multiple(self, db, pratica, user):
        ...
        note = pratica.note_collegate.all()
        assert note.count() == 2
    ...
    def test_relazioni_tra_pratiche(
        ...
        relazione = PraticaRelazione.objects.create(
            parent=pratica_principale,
            child=pratica_collegata,
            tipo='dipendenza'
        )

        assert relazione.parent == pratica_principale
        assert relazione.child == pratica_collegata
...
class TestFlussoCreazioneCompletaArchivio:
    ...
    def test_flusso_archivio_completo(
        ...
        documento_tipo, unita_archivio, user
    ):
        """
        ...
        scadenza = Scadenza.objects.create(
            ...
            data_scadenza=(timezone.now() + timedelta(days=365)).date(),
            stato='attiva',
        )
        scadenza.pratiche.add(pratica)
        ...
        assert pratica.note_collegate.count() == 1
        assert pratica.scadenze.count() == 1

        # Verifica tutti i documenti protocollati
        for doc in documenti:
            movimenti = MovimentoProtocollo.objects.filter(documento=doc)
