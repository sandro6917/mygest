...
import CollegaDocumentiDialog from '@/components/fascicoli/CollegaDocumentiDialog';
import {
  ArrowBackIcon,
  EditIcon,
  DeleteIcon,
...
export default function FascicoloDetailPage() {
  ...
  const [showCollegaDocumentiDialog, setShowCollegaDocumentiDialog] = useState(false);
  ...
  const loadFascicoliCollegabiliM2M = useCallback(async (search?: string) => {
    ...
    // Se non c'è un criterio di ricerca, svuota l'elenco
    ...
    <div className="page-container">
      ...
      <Box sx={{ borderBottom: 1, borderColor: 'divider', marginBottom: 3 }}>
        ...
        >
          ...
          <Tab 
            label={
              <Badge badgeContent={sottofascicoli.length} color="primary">
                <span style={{ marginRight: sottofascicoli.length ? '20px' : '0' }}>Sottofascicoli</span>
              </Badge>
            } 
            {...a11yProps(2)} 
          />
          <Tab 
            label={
              <Badge badgeContent={(fascicolo?.pratiche_details?.length || 0) + (fascicolo?.fascicoli_collegati_details?.length || 0)} color="primary">
                <span style={{ marginRight: ((fascicolo?.pratiche_details?.length || 0) + (fascicolo?.fascicoli_collegati_details?.length || 0)) ? '20px' : '0' }}>Relazioni</span>
              </Badge>
            } 
            {...a11yProps(3)} 
          />
          <Tab 
            label={
              <Badge badgeContent={fascicolo?.movimenti_protocollo?.length || 0} color="primary">
                <span style={{ marginRight: fascicolo?.movimenti_protocollo?.length ? '20px' : '0' }}>Protocollo</span>
              </Badge>
            } 
            {...a11yProps(4)} 
          />
      ...
      <div className="card" style={{ marginBottom: '1.5rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
          ...
          <div style={{ display: 'flex', gap: '0.5rem' }}>
            <button
              onClick={() => setShowCollegaDocumentiDialog(true)}
              className="btn-primary"
              style={{ backgroundColor: '#17a2b8' }}
              title="Collega documenti esistenti non fascicolati"
            >
              <AddIcon size={18} />
              Collega Documenti
            </button>
      ...
      {/* Tab 3: Relazioni (Pratiche e Fascicoli Collegati) */}
        ...
        {fascicolo.movimenti_protocollo && fascicolo.movimenti_protocollo.length > 0 ? (
          <div className="table-container">
            <table className="table">
              ...
              <tbody>
                {fascicolo.movimenti_protocollo.map((movimento) => (
                  <tr key={movimento.id}>
                    <td>
                      <span style={{
                        padding: '0.25rem 0.5rem',
                        borderRadius: '0.25rem',
                        fontSize: '0.875rem',
                        backgroundColor: movimento.direzione === 'IN' ? '#d4edda' : '#fff3cd',
                        color: movimento.direzione === 'IN' ? '#155724' : '#856404'
                      }}>
                        {movimento.direzione_display}
                      </span>
                    </td>
                    <td>{new Date(movimento.data).toLocaleDateString('it-IT')}</td>
                    <td>{movimento.protocollo_label}</td>
                    <td>{movimento.destinatario || '-'}</td>
      ...
      {/* Dialog per collegare documenti */}
      {fascicolo && fascicolo.cliente && (
        <CollegaDocumentiDialog
          open={showCollegaDocumentiDialog}
          onClose={() => setShowCollegaDocumentiDialog(false)}
          fascicoloId={fascicolo.id}
          clienteId={fascicolo.cliente}
          onSuccess={() => {
            loadDocumenti(); // Ricarica la lista documenti
            alert('Documenti collegati con successo!');
          }}
        />
      )}
    </div>
