...
export function DocumentoDetailPage() {
  ...
  const handleStampaEtichettaProtocollo = async (movimentoId: number) => {
    try {
      // Fetch del PDF tramite API (con JWT token negli headers)
      const response = await fetch(`/etichette/stampa/ET_PROT_DOC/?movimento_id=${movimentoId}`, {
        headers: {
      ...
      // Libera la memoria dopo un po' (opzionale)
    ...
    <div className="page-container">
      ...
      <Box sx={{ borderBottom: 1, borderColor: 'divider', marginBottom: 3 }}>
        <Tabs
          ...
          variant="scrollable"
          scrollButtons="auto"
          aria-label="tabs documento"
        >
          <Tab label="Dettagli" {...a11yProps(0)} />
          <Tab 
            label={
              <Badge badgeContent={documento.movimenti_protocollo?.length || 0} color="primary">
                <span style={{ marginRight: documento.movimenti_protocollo?.length ? '20px' : '0' }}>
                  Protocollo
                </span>
              </Badge>
            } 
            ...
            <div style={{ marginBottom: '1rem', padding: '1rem', backgroundColor: '#e7f3ff', borderRadius: '0.375rem', border: '1px solid #b3d9ff' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                ...
                <span className={`badge ${documento.protocollo_attivo.direzione === 'IN' ? 'badge-success' : 'badge-warning'}`}>
                  {documento.protocollo_attivo.direzione === 'IN' ? 'Entrata ⬅️' : 'Uscita ➡️'}
                </span>
              </div>
              ...
              {!documento.protocollo_attivo.chiuso && documento.protocollo_attivo.direzione === 'OUT' && (
                <div style={{ marginTop: '0.5rem' }}>
                  <span className="badge badge-warning">⚠️ In prestito</span>
                </div>
              )}
            </div>
          )}
            ...
            <div style={{ marginBottom: '1rem', padding: '1rem', backgroundColor: '#f8f9fa', borderRadius: '0.375rem' }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem' }}>
                ...
                <div className="form-group">
                  ...
                  <p style={{ marginTop: '0.25rem' }}>
                    {documento.protocollo_direzione === 'IN' ? 'Entrata ⬅️' : 'Uscita ➡️'}
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <button 
                onClick={() => navigate(`/scadenze?collega_documento=${documento.id}`)} 
                className="btn-secondary"
                title="Collega una scadenza esistente"
              >
                <AddIcon size={18} />
                Collega Scadenza
              </button>
          ...
          <div style={{ textAlign: 'center', padding: '2rem', color: '#6b7280' }}>
            <p>Funzionalità scadenze in fase di implementazione.</p>
            <p style={{ fontSize: '0.875rem', marginTop: '0.5rem' }}>
              Utilizza i pulsanti sopra per creare o collegare una scadenza.
            </p>
          </div>
        </div>
      </TabPanel>
