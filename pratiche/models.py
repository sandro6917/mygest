...
class Pratica(models.Model):
    ...
    genitori = models.ManyToManyField(
        ...
        symmetrical=False,
        related_name="pratiche_figlie",  # Cambiato qui
        blank=True,
    ...
    figli = models.ManyToManyField(
        ...
        symmetrical=False,
        related_name='pratiche_genitrici',  # Cambiato qui
        blank=True,
    )

    # 🔹 Dati principali
    codice = models.CharField(max_length=60, unique=True, blank=True)
    cliente = models.ForeignKey(Cliente, on_delete=models.PROTECT, related_name="pratiche")
    ...
    def _next_progressivo(self) -> int:
        qs = (
            ...
            .filter(
                ...
                periodo_key=self.periodo_key,
            )
            .order_by("-progressivo")
        )
        if self.pk:
            qs = qs.exclude(pk=self.pk)
        last = qs.values_list("progressivo", flat=True).first()
        return (last or 0) + 1

    @property
    def anagrafica(self) -> Anagrafica:
        # scorciatoia: l’anagrafica del Cliente collegato
        return self.cliente.anagrafica

    def _format_codice(self, seq: int) -> str:
        # get_or_generate_cli lavora su Anagrafica
        cli_code = get_or_generate_cli(self.anagrafica)
    ...
    def save(self, *args, **kwargs):
        ...
        new_periodo_key = self._build_periodo_key()
        self.periodo_key = new_periodo_key
        ...
        if not creating and self.pk:
            original_group = (
                ...
                .filter(pk=self.pk)
                .values_list("cliente_id", "tipo_id", "periodo_key")
                .first()
            )

        self.full_clean()

        current_group = (self.cliente_id, self.tipo_id, self.periodo_key)
        group_changed = creating or (original_group is None) or (current_group != original_group)

        if group_changed or not self.progressivo:
    ...
    def __str__(self):
        return f"{self.codice} - {self.oggetto} ({self.get_stato_display()})"
    ...
    def get_or_create_fascicolo(self, *, titolo=None, titolario_voce=None, anno=None):
        ...
        fasc = Fascicolo.objects.create(
            ...
            anno=anno,
            cliente=self.cliente,  # ora coerente (FK a Cliente)
        )
        return fasc, True
...
# Note
# ==============================
class PraticaNota(models.Model):
    class Tipo(models.TextChoices):
        MEMO = "memo", _("Memo")
        COMUNICAZIONE = "comunicazione", _("Comunicazione")
        CHIUSURA = "chiusura", _("Chiusura")
        ALTRO = "altro", _("Altro")
    ...
    pratica = models.ForeignKey("Pratica", on_delete=models.CASCADE, related_name="note_collegate")
    tipo = models.CharField(max_length=32, choices=Tipo.choices, default=Tipo.MEMO)
    testo = models.TextField()
    data = models.DateField(default=timezone.now)
    stato = models.CharField(max_length=16, choices=Stato.choices, default=Stato.APERTA)

    class Meta:
        verbose_name = _("Nota pratica")
        verbose_name_plural = _("Note pratica")
        ordering = ["-data", "-id"]
        indexes = [
            models.Index(fields=["pratica", "data"]),
        ]

    def __str__(self):
        return f"{self.pratica.codice} · {self.get_tipo_display()} · {self.data:%d/%m/%Y}"
